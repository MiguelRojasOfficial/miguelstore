...
export default function CategoryPage() {
  ...
  useEffect(() => {
    const filtered = mockProducts.filter((p) => p.category.toLowerCase() === slug)
  ...
  if (products.length === 0)
    return <div className="text-center py-20">No hay productos en la categoría "{slug}"</div>

  return (
    <div className="bg-gray-50 min-h-screen px-6 md:px-16 py-12">
      <h1 className="text-4xl font-bold text-red-600 mt-5 mb-12 text-center">
        {slug.toUpperCase()}
      </h1>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
        {products.map((prod) => (
          <div key={prod.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition relative group">
            <Link href={`/productos/${prod.slug}`}>
              <Image src={prod.image} alt={prod.title} width={580} height={580} className="w-full h-48 object-contain bg-white transition-transform duration-300 group-hover:scale-105" />
            </Link>

            <button className="absolute top-2 right-2 z-10" onClick={(e) => { e.preventDefault(); toggleFavorite(prod) }}>
              <Heart className={`w-6 h-6 ${favoriteIds.includes(prod.id) ? 'text-red-500' : 'text-gray-400'}`} />
            </button>

            <div className="p-4 flex flex-col">
              <h3 className="font-semibold text-gray-400 text-sm">{prod.title}</h3>
              <p className="text-gray-700 font-medium">${prod.offerPrice ? prod.offerPrice.toFixed(2) : prod.price.toFixed(2)}</p>
              <button className="mt-4 w-full bg-red-600 text-white py-2 rounded-lg font-semibold hover:bg-red-700 transition" onClick={(e) => { e.preventDefault(); addToCart(prod) }}>
                Agregar a mi bolsa
              </button>
            </div>
          </div>
